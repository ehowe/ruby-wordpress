class CreateRwPosts < ActiveRecord::Migration
  def self.up
    create_table :rw_posts do |t|
  ...
  def self.down
    remove_index :rw_posts, :name => :post_name
    remove_index :rw_posts, :name => :type_status_date
    remove_index :rw_posts, :name => :post_parent
    remove_index :rw_posts, :name => :post_author
    drop_table :rw_posts
  end
end
